...
def selenium_driver():
    ...
    driver = webdriver.Chrome(executable_path=r"/Users/alighterness/Downloads/chromedriver", options=options)
    return driver
...
def main(db_cred):
    # Get image ID's
    #image_id = get_img_files(img_directory)
    # Send images to Tineye and Scrape Results
    data_tineye_list = get_image_from_db_reverse(db_cred)
    data_tineye = search_tineye(data_tineye_list)
